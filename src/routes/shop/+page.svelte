		...
		<div class="flex flex-col md:flex-row gap-8">
			...
			<div class="flex-1">
					...
					{#if filteredProjects.length > 0}
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							{#each paginatedProjects as project}
								<div style={`view-transition-name: product-${project.id}`}
										 class="bg-slate-800 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300">
				...
				<div class="mt-8 p-4 bg-slate-800 rounded-lg text-slate-400 text-sm">
					<p>Showing {paginatedProjects.length} of {filteredProjects.length} projects (Page {currentPage}
						of {totalPages || 1})</p>
				</div>
			</div>
		</div>
